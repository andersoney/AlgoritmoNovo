...
#include <math.h>
#include <string.h>
#include "createScenarioConfig.h"
...
int main(int argc, char **argv)
{
   if (argc < 4)
   {
      ...
      cerr << "Use: " << endl;
      cerr << "createScenario <file> <numRobots> <prob> [video]" << endl;
      exit(1);
   }

   ofstream output((string(argv[1])+".world").c_str());
   int numRobots = atoi(argv[2]);
   double PROB_ANGER = atof(argv[3]);
   ...
   for(int i = 0; i < numRobots; i++)
   {
      ...
      output << "robot" << endl
	     ...
	     << "  ctrl \"coordination.so " << i << " " << numRobots << " " << PROB_ANGER << "\"" << endl
	     << ")" << endl << endl;
